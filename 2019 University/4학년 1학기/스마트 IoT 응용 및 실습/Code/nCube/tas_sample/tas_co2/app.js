...
var a = 0; //과제는 number_upload_action에서 실행한다.'
var sensor = 0;

function number_upload_action() {//작동
    if (tas_state == 'upload') {//tas에서 업로드가 되면
        fs.readFile('/home/pi/nCube/tas_sample/tas_co2/text.txt','utf-8',function(err,data){
            console.log(data);
            sensor = data;
         });
        var con = { value: sensor };//컨테이너 만들기

        for (var i = 0; i < upload_arr.length; i++) {
            if (upload_arr[i].id == 'number') {
                var cin = { ctname: upload_arr[i].ctname, con: con.value };//컨테이너이름하고 컨테이너값 묶어서 cin으로 만들고
                console.log(JSON.stringify(cin) + ' ---->');//cin을 json 형식으로 바꿔주고
                upload_client.write(JSON.stringify(cin) + '<EOF>');//json을 client(소켓)을 통해서 전송
                a++;
                break;
            }
...
var SerialPort = null;
var myPort = null;
function tas_watchdog() {       // 소켓을 통해서 데이터를 보낸다. thyme - conf.js로 보낸다.
    if (tas_state == 'init') {
...
wdt.set_wdt(require('shortid').generate(), 2, number_upload_action);
